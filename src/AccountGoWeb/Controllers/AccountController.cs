...
public class AccountController : GoodController
{
    ...
    public async Task<IActionResult> SignIn(LoginViewModel model, string? returnUrl = null)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            HttpResponseMessage responseSignIn = await PostAsync("account/signin", content);
            string responseJson = await responseSignIn.Content.ReadAsStringAsync();

            if (responseSignIn.IsSuccessStatusCode && !string.IsNullOrWhiteSpace(responseJson))
            {
                var resultSignIn = Newtonsoft.Json.Linq.JObject.Parse(responseJson);
